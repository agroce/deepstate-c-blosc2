...
TEST(CBlosc, RoundTrip) {
  // These tests are for the original cblosc.  We still get values for delta, to make tests binary compatible.
  ...
  for (int i = 0; i < num_runs; i++) {
    ...
    for (int j = 0; j < num_internal_actions; j++) {
      OneOf(
	    ...
	    [&] {
	      int new_delta = DeepState_UIntInRange(0, 1);
	      LOG(TRACE) << "Setting delta to " << new_delta << "(NOP for original blosc)";
	      //blosc_set_delta(new_delta);
	    },
	    [&] {
